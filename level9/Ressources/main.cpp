...
class N { //the total size of the class might be 108? (because of new having a size of 108)
	//angr seem to suggest that this class has 2 values, 1 of type int and one of type char*
	char annotation[104]; //the order of those values is unclear, because sometimes they are accessed by negative offset to the stack, and sometimes relative to each other with a positive offset
	int value;
...
public:
	...
	char *setAnnotation(char *arg) { //ghidra prototypes this as: N::setAnnotation(N *this,char *param_1), and the memcpy is memcpy(this + 4,param_1,__n), this suggest that annotation is second in memory, after value
		return (char *)memcpy(this->annotation, arg, strlen(arg));
	}
